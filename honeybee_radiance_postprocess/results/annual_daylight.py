...
class AnnualDaylight(Results):
    ...
    def dynamic_schedule_from_sensor_maximum(
        ...
        for grid_info in grids_info:
            ...
            fff = {key: self.valid_states[key] for key in light_paths if key in self.valid_states}

            keys, values = zip(*fff.items())
            ...
            print(np.where(np.all(array_combinations == -np.inf, axis=0)))
